            ...
            <div class="col-lg-12">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="{{ route('borrowings.create') }}" class="btn btn-sm btn-success"><i
                                class="ri-add-circle-line"></i> Make a Borrowing</a>
                                ...
                                @foreach ($borrowings as $borrowing)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="badge bg-success">Returned at
                                                    {{ date('d F Y', strtotime($borrowing->return_date)) }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if (is_null($borrowing->return_date))
                                            ...
                                            <button class="btn btn-danger btn-sm" title="Delete"
                                                onclick="confirmationDialog('delete-form-{{ $borrowing->id }}', 'Are you sure?', 'This book will be permanently deleted!', 'Yes, delete it!')"><i
                                                    class="bi bi-trash"></i></button>

                                            <form id="delete-form-{{ $borrowing->id }}"
                                                action="{{ route('borrowings.destroy', $borrowing->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
